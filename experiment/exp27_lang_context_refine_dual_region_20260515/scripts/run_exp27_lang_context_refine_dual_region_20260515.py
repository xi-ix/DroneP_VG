...
EXP_ROOT = ROOT / 'experiment/exp27_lang_context_refine_dual_region_20260515'
...
BEST_CKPT = LOG_DIR / 'exp27_lang_context_refine_dual_region_20260515_best.pt'
SUMMARY_JSON = LOG_DIR / 'exp27_lang_context_refine_dual_region_20260515_summary.json'
SUMMARY_TRAINVAL_MD = LOG_DIR / 'evaluation_summary_exp27_lang_dual_region_trainval_class_aware.md'
SUMMARY_VAL_MD = LOG_DIR / 'evaluation_summary_exp27_lang_dual_region_val_class_aware.md'
SUMMARY_TEST_MD = LOG_DIR / 'evaluation_summary_exp27_lang_dual_region_test_class_aware.md'
SUMMARY_FULL_MD = LOG_DIR / 'evaluation_summary_exp27_lang_dual_region_full_class_aware.md'
SEARCH_CSV = LOG_DIR / 'postprocess_search_exp27_lang_dual_region.csv'
...
def load_exp23_helpers():
    spec = importlib.util.spec_from_file_location('exp23_helpers_for_exp27', str(EXP23_SCRIPT))
    if spec is None or spec.loader is None:
        raise RuntimeError(f'Cannot import Exp23 helpers from {EXP23_SCRIPT}')
    module = importlib.util.module_from_spec(spec)
    sys.modules['exp23_helpers_for_exp27'] = module
...
def hashed_language_embedding(class_id: int) -> List[float]:
    prompt = CLASS_PROMPTS.get(class_id, CLASS_PROMPTS[11])
    values = [0.0] * LANG_HASH_DIM
    for token in prompt.split():
        bucket = sum(ord(ch) for ch in token) % LANG_HASH_DIM
...
def language_prior_features(class_id: int, area: float, context_contrast: float, refine_contrast: float) -> List[float]:
    is_small_target = 1.0 if class_id in SMALL_TARGET_CLASSES else 0.0
    is_vehicle = 1.0 if class_id in VEHICLE_CLASSES else 0.0
    is_person = 1.0 if class_id in PERSON_CLASSES else 0.0
    ...
    ] + hashed_language_embedding(class_id)


def feature_from_box(box: DetBox, width: int, height: int, image: Image.Image) -> List[float]:
    ...
    language = language_prior_features(box.class_id, area, context[-1], refine[-1])
    return base + context + refine + delta + ratio + language
...
def load_record(split: str, stem: str):
    with Image.open(image_path(split, stem)) as raw_image:
        ...
        feats = [feature_from_box(box, width, height, image) for box in candidates]
...
def main() -> None:
    ...
    log('[Start] Exp27 language-aware context-refine dual-region distillation')
    log(
    ...
    write_summary(SUMMARY_TRAINVAL_MD, 'Exp27 TrainVal Language Dual-Region Distill Summary', trainval_metrics)
    write_summary(SUMMARY_VAL_MD, 'Exp27 Val Language Dual-Region Distill Summary', val_metrics)
    write_summary(SUMMARY_TEST_MD, 'Exp27 Test Language Dual-Region Distill Summary', test_metrics)
    write_summary(SUMMARY_FULL_MD, 'Exp27 Full Language Dual-Region Distill Summary', full_metrics)
    ...
    summary = {
        ...
        'config': {
            ...
            'nms_grid': NMS_GRID,
            'language_hash_dim': LANG_HASH_DIM,
            'feature_design': 'box geometry + A context RGB stats + B refined RGB stats + B-A deltas + class prompt hash + semantic priors',
        },
