...
EXP_ROOT = ROOT / 'experiment/exp25_ranked_teacher_distill_20260515'
...
BEST_CKPT = LOG_DIR / 'exp25_ranked_teacher_distill_20260515_best.pt'
SUMMARY_JSON = LOG_DIR / 'exp25_ranked_teacher_distill_20260515_summary.json'
SUMMARY_TRAINVAL_MD = LOG_DIR / 'evaluation_summary_exp25_ranked_teacher_distill_trainval_class_aware.md'
SUMMARY_VAL_MD = LOG_DIR / 'evaluation_summary_exp25_ranked_teacher_distill_val_class_aware.md'
SUMMARY_TEST_MD = LOG_DIR / 'evaluation_summary_exp25_ranked_teacher_distill_test_class_aware.md'
SUMMARY_FULL_MD = LOG_DIR / 'evaluation_summary_exp25_ranked_teacher_distill_full_class_aware.md'
...
def load_exp23_helpers():
    spec = importlib.util.spec_from_file_location('exp23_helpers_for_exp25', str(EXP23_SCRIPT))
    if spec is None or spec.loader is None:
        raise RuntimeError(f'Cannot import Exp23 helpers from {EXP23_SCRIPT}')
    module = importlib.util.module_from_spec(spec)
    sys.modules['exp23_helpers_for_exp25'] = module
...
def build_rank_pairs(records, offsets: Sequence[int]) -> List[Tuple[int, int]]:
    pairs: List[Tuple[int, int]] = []
    rng = random.Random(SEED)
    for record, offset in zip(records, offsets):
        positives = []
        negatives = []
        for idx, target in enumerate(record['targets']):
            global_idx = offset + idx
            if target >= 0.5:
                positives.append(global_idx)
            elif target <= 0.05:
                negatives.append(global_idx)
        if not positives or not negatives:
            continue
        local_pairs = [(p, n) for p in positives for n in negatives]
        rng.shuffle(local_pairs)
        pairs.extend(local_pairs[:MAX_RANK_PAIRS_PER_IMAGE])
    return pairs


def record_offsets(records) -> List[int]:
    offsets: List[int] = []
    cursor = 0
    for record in records:
        offsets.append(cursor)
        cursor += len(record['features'])
    return offsets
...
def train_model(train_records, val_records):
    ...
    rank_pairs = build_rank_pairs(train_records, record_offsets(train_records))
    rank_pos = torch.tensor([pair[0] for pair in rank_pairs], dtype=torch.long)
    rank_neg = torch.tensor([pair[1] for pair in rank_pairs], dtype=torch.long)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for xb, y_gt, y_soft, y_target in loader:
            ...
            if len(rank_pairs) > 0:
                sample_count = min(8192, len(rank_pairs))
                sample_idx = torch.randint(0, len(rank_pairs), (sample_count,), dtype=torch.long)
                pos_logits = model(x_train[rank_pos[sample_idx]])
                neg_logits = model(x_train[rank_neg[sample_idx]])
                rank_loss = torch.relu(RANK_MARGIN - (pos_logits - neg_logits)).mean()
                loss = loss + BETA_RANKING * rank_loss
        ...
        log(
            ...
            f"rank_loss={sum(rank_losses)/max(1, len(rank_losses)):.6f} "
...
def main() -> None:
    ...
    log('[Start] Exp25 ranked teacher distillation')
    log(
        f'[Config] threshold={THRESHOLD:.2f} nms={NMS_THRESHOLD:.2f} '
        f'epochs={EPOCHS} alpha_teacher={ALPHA_TEACHER:.2f} '
        f'beta_ranking={BETA_RANKING:.2f} margin={RANK_MARGIN:.2f}'
    )
    ...
    write_summary(SUMMARY_TRAINVAL_MD, 'Exp25 TrainVal Ranked Distill Summary', trainval_metrics)
    write_summary(SUMMARY_VAL_MD, 'Exp25 Val Ranked Distill Summary', val_metrics)
    write_summary(SUMMARY_TEST_MD, 'Exp25 Test Ranked Distill Summary', test_metrics)
    write_summary(SUMMARY_FULL_MD, 'Exp25 Full Ranked Distill Summary', full_metrics)
    ...
    summary = {
        ...
        'config': {
            ...
            'beta_ranking': BETA_RANKING,
            'rank_margin': RANK_MARGIN,
            'max_rank_pairs_per_image': MAX_RANK_PAIRS_PER_IMAGE,
        },
        'best': best,
