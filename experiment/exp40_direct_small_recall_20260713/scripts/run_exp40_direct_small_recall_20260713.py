...
EXP_ROOT = ROOT / 'experiment/exp40_direct_small_recall_20260713'
...
SUMMARY_JSON = LOG_DIR / 'exp40_direct_small_recall_20260713_summary.json'
...
SMALL_ALIAS_BOX_THRESHOLD = 0.015
SMALL_ALIAS_TEXT_THRESHOLD = 0.10
...
def generate_online_candidates(gdino_model, image_path: Path) -> List[Candidate]:
    ...
    alias_prompts = [prompt for _, prompt in SMALL_ALIAS_PROMPTS]
    boxes, scores, raw_class_ids = gdino_predict(gdino_model, image_bgr, alias_prompts, SMALL_ALIAS_BOX_THRESHOLD, SMALL_ALIAS_TEXT_THRESHOLD)
    for box, score, raw_cid in zip(boxes, scores, raw_class_ids):
        if raw_cid is None:
            continue
        prompt_idx = int(raw_cid)
        if prompt_idx < 0 or prompt_idx >= len(SMALL_ALIAS_PROMPTS):
            continue
        target_class, prompt = SMALL_ALIAS_PROMPTS[prompt_idx]
        candidates.append(
            Candidate(
                target_class,
                float(box[0]),
...
def run_dataset(split: str, limit: Optional[int]) -> None:
    ...
    log(f'[Start] Exp40 direct small-object recall inference split={split} limit={limit}')
    ...
    if eval_gt_dir.exists():
        ...
        summary = {
            ...
            'config': {
                ...
                'small_alias_box_threshold': SMALL_ALIAS_BOX_THRESHOLD,
                'small_alias_text_threshold': SMALL_ALIAS_TEXT_THRESHOLD,
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Exp40 direct small-object recall online grounding inference')
    ...
    RUN_LOG.write_text('', encoding='utf-8')
    log('[Start] Exp40 online single-image direct-recall inference')
    gdino_model = load_groundingdino_model()
    scorer_model, mu, sigma, vocab = load_scorer()
