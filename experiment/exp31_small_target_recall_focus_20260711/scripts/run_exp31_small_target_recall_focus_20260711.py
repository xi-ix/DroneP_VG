...
EXP_ROOT = ROOT / 'experiment/exp31_small_target_recall_focus_20260711'
...
BEST_CKPT = LOG_DIR / 'exp31_small_target_recall_focus_20260711_best.pt'
SUMMARY_JSON = LOG_DIR / 'exp31_small_target_recall_focus_20260711_summary.json'
SUMMARY_TRAINVAL_MD = LOG_DIR / 'evaluation_summary_exp31_small_target_recall_focus_trainval_class_aware.md'
SUMMARY_VAL_MD = LOG_DIR / 'evaluation_summary_exp31_small_target_recall_focus_val_class_aware.md'
SUMMARY_TEST_MD = LOG_DIR / 'evaluation_summary_exp31_small_target_recall_focus_test_class_aware.md'
SUMMARY_FULL_MD = LOG_DIR / 'evaluation_summary_exp31_small_target_recall_focus_full_class_aware.md'
SEARCH_CSV = LOG_DIR / 'postprocess_search_exp31_small_target_recall_focus.csv'
...
SMALL_POS_WEIGHT = 2.0
WEAK_POS_WEIGHT = 4.0
SMALL_NEG_WEIGHT = 1.15
WEAK_NEG_WEIGHT = 1.35
SMALL_VAL_OBJECTIVE_WEIGHT = 0.35
...
CANONICAL_QUERY.update(CANONICAL_QUERY_OVERRIDE)
...
def load_exp23_helpers():
    spec = importlib.util.spec_from_file_location('exp23_helpers_for_exp31', str(EXP23_SCRIPT))
    if spec is None or spec.loader is None:
        raise RuntimeError(f'Cannot import Exp23 helpers from {EXP23_SCRIPT}')
    module = importlib.util.module_from_spec(spec)
    sys.modules['exp23_helpers_for_exp31'] = module
...
def sample_weight_for(query_class: int, target: float) -> float:
    is_positive = target >= 0.5
    if query_class in WEAK_SMALL_CLASSES:
        return WEAK_POS_WEIGHT if is_positive else WEAK_NEG_WEIGHT
    if query_class in SMALL_TARGET_CLASSES:
        return SMALL_POS_WEIGHT if is_positive else SMALL_NEG_WEIGHT
    return 1.0
...
def write_grounding_summary(path: Path, metrics: Dict[str, object]) -> None:
    lines = [
        '# Exp31 Grounding TopK Evaluation',
        '',
...
def small_class_recall_at_1(metrics: Dict[str, object]) -> float:
    total = 0.0
    weighted = 0.0
    by_class = metrics.get('by_class', {})
    for class_id in sorted(WEAK_SMALL_CLASSES):
        item = by_class.get(str(class_id), {})
        queries = float(item.get('queries', 0.0))
        total += queries
        weighted += queries * float(item.get('recall_at_1', 0.0))
...
def train_model(train_records, val_records, vocab: Dict[str, int]):
    x_train_raw, tok_train, gt_train, soft_train, target_train, weight_train, class_train = flatten(train_records, vocab, 'train')
    x_val_raw, _, _, _, _, _, _ = flatten(val_records, vocab, 'val')
    ...
    loader = DataLoader(
        TensorDataset(x_train, tok_train, gt_train, soft_train, target_train, weight_train, class_train),
        batch_size=BATCH_SIZE,
        shuffle=True,
        pin_memory=(DEVICE.type == 'cuda'),
    )

    best = {'epoch': 0, 'val_objective': -1.0, 'val_acc_05': 0.0, 'val_map_05': 0.0, 'val_small_r1': 0.0}
    for epoch in range(1, EPOCHS + 1):
        model.train()
        losses = []
        for xb, token_ids, y_gt, y_soft, y_target, sample_weight, class_id in loader:
            ...
            bce_gt = nn.functional.binary_cross_entropy_with_logits(logits, y_gt, reduction='none')
            bce_soft = nn.functional.binary_cross_entropy_with_logits(logits, y_target, reduction='none')
            bce_loss = ((bce_gt + ALPHA_TEACHER * bce_soft) * sample_weight).mean()
            focal_loss = focal_bce_loss(logits, y_target, sample_weight)
            rank_loss = batch_ranking_loss(logits, y_target, class_id)
        ...
        val_grounding = evaluate_grounding_topk(model, val_records, mu, sigma, vocab)
        val_small_r1 = small_class_recall_at_1(val_grounding)
        val_det_objective = 0.5 * val_metrics['acc_05'] + 0.5 * val_metrics['map_05']
        val_objective = (1.0 - SMALL_VAL_OBJECTIVE_WEIGHT) * val_det_objective + SMALL_VAL_OBJECTIVE_WEIGHT * val_small_r1
        log(
            f"[Epoch] {epoch:03d}/{EPOCHS} "
            f"loss={sum(losses)/max(1, len(losses)):.6f} "
            f"val_acc05={val_metrics['acc_05']:.4f} "
            f"val_map05={val_metrics['map_05']:.4f} "
            f"val_small_r1={val_small_r1:.4f} "
...
def focal_bce_loss(logits: torch.Tensor, targets: torch.Tensor, weights: torch.Tensor, gamma: float = FOCAL_GAMMA) -> torch.Tensor:
    bce = nn.functional.binary_cross_entropy_with_logits(logits, targets, reduction='none')
    probs = torch.sigmoid(logits)
    pt = probs * targets + (1.0 - probs) * (1.0 - targets)
    return (weights * bce * ((1.0 - pt).clamp(min=1e-6) ** gamma)).mean()


def batch_ranking_loss(logits: torch.Tensor, targets: torch.Tensor, class_ids: torch.Tensor) -> torch.Tensor:
    weak_mask = torch.zeros_like(targets, dtype=torch.bool)
    for class_id in WEAK_SMALL_CLASSES:
        weak_mask |= class_ids == class_id
    pos = logits[(targets >= 0.5) & weak_mask]
    neg = logits[(targets <= 0.05) & weak_mask]
...
def main() -> None:
    ...
    log('[Start] Exp31 small-target recall focused scorer')
    log(
        ...
        f'small_val_objective_weight={SMALL_VAL_OBJECTIVE_WEIGHT:.2f}'
    )
    log(f'[Device] {DEVICE}')
    ...
    write_summary(SUMMARY_TRAINVAL_MD, 'Exp31 TrainVal Small Target Recall Focus Summary', trainval_metrics)
    write_summary(SUMMARY_VAL_MD, 'Exp31 Val Small Target Recall Focus Summary', val_metrics)
    write_summary(SUMMARY_TEST_MD, 'Exp31 Test Small Target Recall Focus Summary', test_metrics)
    write_summary(SUMMARY_FULL_MD, 'Exp31 Full Small Target Recall Focus Summary', full_metrics)
    ...
    summary = {
        ...
        'config': {
            ...
            'small_pos_weight': SMALL_POS_WEIGHT,
            'weak_pos_weight': WEAK_POS_WEIGHT,
            'small_neg_weight': SMALL_NEG_WEIGHT,
            'weak_neg_weight': WEAK_NEG_WEIGHT,
            'small_val_objective_weight': SMALL_VAL_OBJECTIVE_WEIGHT,
            ...
            'feature_design': 'Exp30 multi-scale query scorer + weak/small-class sample weighting + weak-class ranking + small-target validation objective',
        },
