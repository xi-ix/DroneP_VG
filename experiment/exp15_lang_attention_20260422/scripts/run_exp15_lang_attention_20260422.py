...
EXP_ROOT = ROOT / 'experiment/exp15_lang_attention_20260422'
...
SUMMARY_JSON = LOG_DIR / 'exp15_lang_attention_20260422_summary.json'
BEST_CKPT = LOG_DIR / 'exp15_lang_attention_20260422_best.pt'
...
SUMMARY_TRAINVAL_MD = LOG_DIR / 'evaluation_summary_exp15_lang_attention_trainval_class_aware.md'
SUMMARY_TEST_MD = LOG_DIR / 'evaluation_summary_exp15_lang_attention_test_class_aware.md'
SUMMARY_FULL_MD = LOG_DIR / 'evaluation_summary_exp15_lang_attention_full_class_aware.md'
...
class HybridExternalCalibrator(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int = 64, head_dim: int = 32, prompt_vocab_size: int = PROMPT_VOCAB_SIZE, prompt_dim: int = 32, attn_scale: float = 0.5):
        super().__init__()
        self.prompt_embedding = nn.Embedding(prompt_vocab_size, prompt_dim)
        self.prompt_proj = nn.Sequential(
            nn.Linear(prompt_dim, head_dim),
            nn.ReLU(),
            nn.Linear(head_dim, input_dim),
        )
    ...
    def forward(self, x: torch.Tensor, prompt_ids: torch.Tensor) -> torch.Tensor:
        if prompt_ids.numel() == 0:
            gate = torch.ones((x.shape[0], x.shape[1]), dtype=x.dtype, device=x.device)
        else:
            prompt_vec = self.prompt_embedding(prompt_ids.to(x.device)).mean(dim=0)
...
def record_summary(metrics: Dict[str, float]) -> List[str]:
    return [
        '# Exp15 Language Attention Summary',
        '',
...
def train_supervised(model: HybridExternalCalibrator, x_train: torch.Tensor, y_train: torch.Tensor, x_val: torch.Tensor, y_val: torch.Tensor, prompt_ids: torch.Tensor) -> Dict[str, float]:
...
def train_reinforcement(model: HybridExternalCalibrator, train_records, val_records, mu: torch.Tensor, sigma: torch.Tensor, prompt_ids: torch.Tensor) -> Dict[str, float]:
...
def main() -> None:
    ...
    log('[Start] Exp15 language-attention rebuild based on Exp14 pipeline')
