...
SUMMARY_JSON = LOG_DIR / 'exp38_online_rich_metadata_fusion_20260712_summary.json'
...
def run_dataset(split: str, limit: Optional[int]) -> None:
    ...
    log(f'[Start] Exp38 online rich metadata inference split={split} limit={limit}')
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Exp38 online rich-metadata small-object grounding inference')
    ...
    RUN_LOG.write_text('', encoding='utf-8')
    log('[Start] Exp38 online single-image inference')
    gdino_model = load_groundingdino_model()
    scorer_model, mu, sigma, vocab = load_scorer()
