...
BASELINE_JSON = LOG_DIR / 'restricted_search_result_20260421.json'
RESULT_JSON = LOG_DIR / 'local_refine_search_result_20260421.json'
RESULT_MD = LOG_DIR / 'local_refine_search_result_20260421.md'


def load_exp14_module():
    spec = importlib.util.spec_from_file_location('exp14_rebuild_module_local_refine', str(BASE_SCRIPT))
    if spec is None or spec.loader is None:
        raise RuntimeError(f'Cannot load module from {BASE_SCRIPT}')
    module = importlib.util.module_from_spec(spec)
    sys.modules['exp14_rebuild_module_local_refine'] = module
    spec.loader.exec_module(module)
    return module


def evaluate_full(module, pred_by_stem: Dict[str, List[object]]) -> Dict[str, float]:
    full_pred_dir = module.PRED_ROOT / 'full_local_refine'
    if full_pred_dir.exists():
        shutil.rmtree(full_pred_dir)
    module.save_predictions(pred_by_stem, full_pred_dir)

    merged_gt_dir = module.GT_ROOT / 'full_local_refine'
...
def evaluate_val(module, pred_by_stem: Dict[str, List[object]]) -> Dict[str, float]:
    val_pred_dir = module.PRED_ROOT / 'val_local_refine'
...
def build_search_space() -> Dict[str, List[float]]:
    thresholds = [round(0.08 + 0.005 * i, 3) for i in range(21)]
    nms_values = [round(0.52 + 0.02 * i, 2) for i in range(9)]
    return {'thresholds': thresholds, 'nms_values': nms_values}
...
def main() -> None:
    ...
    baseline_acc05 = float(baseline_summary.get('full_metrics', {}).get('acc_05', 0.0))

    previous_best_acc05 = None
    previous_best = None
    if BASELINE_JSON.exists():
        last = json.loads(BASELINE_JSON.read_text(encoding='utf-8'))
        previous_best = last.get('best', {})
        if previous_best:
            previous_best_acc05 = float(previous_best.get('full_acc_05', 0.0))
    ...
    if x_train_raw.numel() == 0:
        raise RuntimeError('No train features loaded for local refine search.')
    ...
    nms_values = search_space['nms_values']
    ...
    delta_vs_full = best['full_acc_05'] - baseline_acc05
    delta_vs_prev = None if previous_best_acc05 is None else best['full_acc_05'] - previous_best_acc05

    output = {
        'base_summary': str(summary_path),
        'restricted_search_baseline': str(BASELINE_JSON) if BASELINE_JSON.exists() else None,
        ...
        'best': best,
        'delta_vs_full_acc_05': delta_vs_full,
        'delta_vs_previous_restricted_best_acc_05': delta_vs_prev,
        'top10': top10,
    }
    ...
    lines = [
        '# Exp14 局部二次复搜结果',
        '',
        f"- 基线 full Acc@0.5: {baseline_acc05:.4f}",
        (
            f"- 上一版受限复搜最优 Acc@0.5: {previous_best_acc05:.4f}"
            if previous_best_acc05 is not None
            else '- 上一版受限复搜最优 Acc@0.5: N/A'
        ),
        f"- 本轮最优 threshold: {best['threshold']:.3f}",
        f"- 本轮最优 nms: {best['nms']:.2f}",
        f"- 本轮最优 full Acc@0.5: {best['full_acc_05']:.4f}",
        f"- 相对 full 基线变化: {delta_vs_full:+.4f}",
        (
            f"- 相对上一版复搜变化: {delta_vs_prev:+.4f}"
            if delta_vs_prev is not None
            else '- 相对上一版复搜变化: N/A'
        ),
        f"- 本轮最优 full Acc@0.75: {best['full_acc_075']:.4f}",
        f"- 本轮最优 full mAP@0.5: {best['full_map_05']:.4f}",
        '',
    ...
    for idx, row in enumerate(top10, start=1):
        lines.append(
            f"| {idx} | {row['threshold']:.3f} | {row['nms']:.2f} | {row['full_acc_05']:.4f} | {row['full_acc_075']:.4f} | {row['full_map_05']:.4f} | {row['val_acc_05']:.4f} | {row['val_map_05']:.4f} |"
        )
    ...
    print(
        f"[Best] threshold={best['threshold']:.3f} nms={best['nms']:.2f} "
        f"full_acc05={best['full_acc_05']:.4f} "
        f"delta_vs_full={delta_vs_full:+.4f}"
    )
    if delta_vs_prev is not None:
        print(f'[Best] delta_vs_previous_restricted={delta_vs_prev:+.4f}')
