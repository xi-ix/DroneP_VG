...
EXP_ROOT = ROOT / 'experiment/exp14_full_rebuild_20260421'
...
SUMMARY_JSON = LOG_DIR / 'exp14_full_rebuild_20260421_summary.json'
BEST_CKPT = LOG_DIR / 'exp14_full_rebuild_20260421_best.pt'
...
SUMMARY_TRAINVAL_MD = LOG_DIR / 'evaluation_summary_exp14_full_rebuild_trainval_class_aware.md'
SUMMARY_TEST_MD = LOG_DIR / 'evaluation_summary_exp14_full_rebuild_test_class_aware.md'
SUMMARY_FULL_MD = LOG_DIR / 'evaluation_summary_exp14_full_rebuild_full_class_aware.md'
...
class HybridExternalCalibrator(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int = 64, head_dim: int = 32):
        super().__init__()
...
def apply_model_to_records(model: nn.Module, records, mu: torch.Tensor, sigma: torch.Tensor, threshold: float, nms_thr: float) -> Dict[str, List[DetBox]]:
    model.eval()
    ...
    with torch.no_grad():
        for record in records:
            ...
            probs = torch.sigmoid(model(feats)).squeeze(1).cpu().tolist()
            selected = []
...
def search_thresholds(model: nn.Module, val_records, mu: torch.Tensor, sigma: torch.Tensor) -> Tuple[float, float, Dict[str, float]]:
    best_score = -1.0
    best_threshold = 0.20
    best_nms = 0.40
    ...
    for threshold in thresholds:
        for nms_thr in nms_values:
            pred = apply_model_to_records(model, val_records, mu, sigma, threshold, nms_thr)
...
def train_supervised(model: HybridExternalCalibrator, x_train: torch.Tensor, y_train: torch.Tensor, x_val: torch.Tensor, y_val: torch.Tensor) -> Dict[str, float]:
    ...
    for epoch in range(1, 61):
        ...
        with torch.no_grad():
            train_acc = float(((torch.sigmoid(model(x_train)).squeeze(1) > 0.5).float() == y_train).float().mean().item())
...
def train_reinforcement(model: HybridExternalCalibrator, train_records, val_records, mu: torch.Tensor, sigma: torch.Tensor) -> Dict[str, float]:
    ...
    for epoch in range(1, 41):
        ...
        with torch.no_grad():
            for record in val_records:
                ...
                feats = torch.tensor(record['features'], dtype=torch.float32)
                feats = (feats - mu) / sigma
                probs = torch.sigmoid(model(feats).squeeze(1)).tolist()
                chosen = []
                for box, prob in zip(record['boxes'], probs):
                    if prob > 0.5:
                        chosen.append(DetBox(box.class_id, box.x1, box.y1, box.x2, box.y2, float(prob), box.source))
...
def main() -> None:
    ...
    log('[Start] Exp14 full rebuild based on Exp13 full predictions + external SL/RL')
    ...
    calibrator = HybridExternalCalibrator(input_dim=x_train.shape[1], hidden_dim=64, head_dim=32)
    sl_best = train_supervised(calibrator, x_train, y_train, x_val, y_val)
    rl_best = train_reinforcement(calibrator, train_records, val_records, mu, sigma)

    ckpt = torch.load(BEST_CKPT, map_location='cpu')
    calibrator.load_state_dict(ckpt['model'], strict=False)

    best_threshold, best_nms, val_search_metrics = search_thresholds(calibrator, val_records, mu, sigma)
    log(f'[Search] threshold={best_threshold:.2f} nms={best_nms:.2f} val_acc05={val_search_metrics.get("acc_05", 0.0):.4f} val_map05={val_search_metrics.get("map_05", 0.0):.4f}')

    trainval_pred = apply_model_to_records(calibrator, val_records, mu, sigma, best_threshold, best_nms)
    test_pred = apply_model_to_records(calibrator, test_records, mu, sigma, best_threshold, best_nms)
    full_pred = apply_model_to_records(calibrator, full_records, mu, sigma, best_threshold, best_nms)
