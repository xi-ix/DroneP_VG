...
EXP_ROOT = ROOT / 'experiment/exp41_grouped_small_recall_20260713'
...
SUMMARY_JSON = LOG_DIR / 'exp41_grouped_small_recall_20260713_summary.json'
...
PERSON_ALIAS_BOX_THRESHOLD = 0.015
PERSON_ALIAS_TEXT_THRESHOLD = 0.10
SMALL_VEHICLE_ALIAS_BOX_THRESHOLD = 0.015
SMALL_VEHICLE_ALIAS_TEXT_THRESHOLD = 0.10
...
SMALL_VEHICLE_ALIAS_PROMPTS = [
    (3, 'bicycle'),
    (3, 'bike'),
    (3, 'cyclist'),
    (7, 'tricycle'),
    (7, 'three wheel vehicle'),
    (7, 'small tricycle'),
    (8, 'covered tricycle'),
    (8, 'awning tricycle'),
    (8, 'canopy tricycle'),
...
SMALL_ALIAS_PROMPTS = PERSON_ALIAS_PROMPTS + SMALL_VEHICLE_ALIAS_PROMPTS
...
def generate_online_candidates(gdino_model, image_path: Path) -> List[Candidate]:
    ...
    boxes, scores, raw_class_ids = gdino_predict(gdino_model, image_bgr, BASE_GDINO_CLASSES, BASE_BOX_THRESHOLD, BASE_TEXT_THRESHOLD)
    ...
    # Keep one batched prompt call. Splitting prompts into separate GroundingDINO
    # calls changed the candidate distribution and hurt recall on val50.
    alias_groups = [
        (SMALL_ALIAS_PROMPTS, SMALL_VEHICLE_ALIAS_BOX_THRESHOLD, SMALL_VEHICLE_ALIAS_TEXT_THRESHOLD),
    ]
    for alias_specs, box_threshold, text_threshold in alias_groups:
        alias_prompts = [prompt for _, prompt in alias_specs]
        boxes, scores, raw_class_ids = gdino_predict(gdino_model, image_bgr, alias_prompts, box_threshold, text_threshold)
        for box, score, raw_cid in zip(boxes, scores, raw_class_ids):
            if raw_cid is None:
                continue
            prompt_idx = int(raw_cid)
            if prompt_idx < 0 or prompt_idx >= len(alias_specs):
                continue
            target_class, prompt = alias_specs[prompt_idx]
            candidates.append(
                Candidate(
                    target_class,
                    float(box[0]),
...
def write_metadata_jsonl(path: Path, image_path: Path, boxes: Sequence[Candidate]) -> None:
    ...
    with path.open('w', encoding='utf-8') as f:
        for rank, box in enumerate(sorted(boxes, key=lambda item: item.score, reverse=True), start=1):
            ...
            source_type = 'alias' if box.source_name.startswith('gdino_alias:') else 'base'
...
def run_dataset(split: str, limit: Optional[int]) -> None:
    ...
    log(f'[Start] Exp41 grouped small-object recall inference split={split} limit={limit}')
    ...
    if eval_gt_dir.exists():
        ...
        summary = {
            ...
            'config': {
                ...
                'base_box_threshold': BASE_BOX_THRESHOLD,
                'base_text_threshold': BASE_TEXT_THRESHOLD,
                'person_alias_box_threshold': PERSON_ALIAS_BOX_THRESHOLD,
                'person_alias_text_threshold': PERSON_ALIAS_TEXT_THRESHOLD,
                'small_vehicle_alias_box_threshold': SMALL_VEHICLE_ALIAS_BOX_THRESHOLD,
                'small_vehicle_alias_text_threshold': SMALL_VEHICLE_ALIAS_TEXT_THRESHOLD,
                'person_alias_prompts': PERSON_ALIAS_PROMPTS,
                'small_vehicle_alias_prompts': SMALL_VEHICLE_ALIAS_PROMPTS,
                'small_alias_prompts': SMALL_ALIAS_PROMPTS,
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Exp41 grouped small-object recall online grounding inference')
    ...
    RUN_LOG.write_text('', encoding='utf-8')
    log('[Start] Exp41 online single-image grouped-recall inference')
    gdino_model = load_groundingdino_model()
    scorer_model, mu, sigma, vocab = load_scorer()
