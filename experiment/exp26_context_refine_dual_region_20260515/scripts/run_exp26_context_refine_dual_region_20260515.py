...
EXP_ROOT = ROOT / 'experiment/exp26_context_refine_dual_region_20260515'
...
BEST_CKPT = LOG_DIR / 'exp26_context_refine_dual_region_20260515_best.pt'
SUMMARY_JSON = LOG_DIR / 'exp26_context_refine_dual_region_20260515_summary.json'
SUMMARY_TRAINVAL_MD = LOG_DIR / 'evaluation_summary_exp26_dual_region_trainval_class_aware.md'
SUMMARY_VAL_MD = LOG_DIR / 'evaluation_summary_exp26_dual_region_val_class_aware.md'
SUMMARY_TEST_MD = LOG_DIR / 'evaluation_summary_exp26_dual_region_test_class_aware.md'
SUMMARY_FULL_MD = LOG_DIR / 'evaluation_summary_exp26_dual_region_full_class_aware.md'
...
def load_exp23_helpers():
    spec = importlib.util.spec_from_file_location('exp23_helpers_for_exp26', str(EXP23_SCRIPT))
    if spec is None or spec.loader is None:
        raise RuntimeError(f'Cannot import Exp23 helpers from {EXP23_SCRIPT}')
    module = importlib.util.module_from_spec(spec)
    sys.modules['exp23_helpers_for_exp26'] = module
...
def main() -> None:
    ...
    log('[Start] Exp26 context-refine dual-region distillation')
    log(
    ...
    write_summary(SUMMARY_TRAINVAL_MD, 'Exp26 TrainVal Dual-Region Distill Summary', trainval_metrics)
    write_summary(SUMMARY_VAL_MD, 'Exp26 Val Dual-Region Distill Summary', val_metrics)
    write_summary(SUMMARY_TEST_MD, 'Exp26 Test Dual-Region Distill Summary', test_metrics)
    write_summary(SUMMARY_FULL_MD, 'Exp26 Full Dual-Region Distill Summary', full_metrics)
    ...
    summary = {
        ...
        'config': {
            ...
            'feature_design': 'box geometry + A context RGB stats + B refined RGB stats + B-A deltas',
        },
        'best': best,
