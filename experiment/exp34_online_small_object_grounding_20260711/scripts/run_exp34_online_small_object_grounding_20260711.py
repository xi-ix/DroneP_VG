...
EXP_ROOT = ROOT / 'experiment/exp34_online_small_object_grounding_20260711'
...
SUMMARY_JSON = LOG_DIR / 'exp34_online_small_object_grounding_20260711_summary.json'
...
def predict_image_from_candidates(
    ...
    scored: List[Candidate] = []
    if target_class:
        scored.extend(score_candidates(scorer_model, mu, sigma, vocab, image_path, candidates, query, target_class))
    else:
        for class_id, class_query in CANONICAL_QUERY.items():
            scored.extend(score_candidates(scorer_model, mu, sigma, vocab, image_path, candidates, class_query, class_id))
    filtered = [box for box in scored if box.score >= score_threshold]
    return class_aware_nms(filtered, nms_threshold)
...
def run_dataset(split: str, limit: Optional[int]) -> None:
    ...
    log(f'[Start] Exp34 online dataset inference split={split} limit={limit}')
    ...
    if eval_gt_dir.exists():
        ...
        SUMMARY_JSON.write_text(json.dumps({'split': split, 'limit': limit, 'metrics': metrics}, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
        log(f"[Done] acc05={metrics['acc_05']:.4f} acc075={metrics['acc_075']:.4f} map05={metrics['map_05']:.4f}")


def main() -> None:
    parser = argparse.ArgumentParser(description='Exp34 online small-object grounding inference')
    ...
    RUN_LOG.write_text('', encoding='utf-8')
    log('[Start] Exp34 online single-image inference')
    gdino_model = load_groundingdino_model()
    scorer_model, mu, sigma, vocab = load_scorer()
