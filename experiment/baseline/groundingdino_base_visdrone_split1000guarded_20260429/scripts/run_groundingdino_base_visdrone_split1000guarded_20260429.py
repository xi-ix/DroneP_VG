...
EXP_ROOT = ROOT / 'experiment/baseline/groundingdino_base_visdrone_split1000guarded_20260429'
...
SUMMARY_MD = LOG_DIR / 'evaluation_summary_groundingdino_base_visdrone_split1000guarded_20260429_class_aware.md'
SUMMARY_JSON = LOG_DIR / 'groundingdino_base_visdrone_split1000guarded_20260429_summary.json'
...
def build_split_index() -> Dict[str, Tuple[Path, Path, str]]:
    index: Dict[str, Tuple[Path, Path, str]] = {}
    for split in ['train', 'val', 'test']:
        image_dir = DATA_ROOT / f'VisDrone2019-DET-{split}' / 'images'
        ...
        for ann_path in sorted(ann_dir.glob('*.txt')):
            ...
            if image_path.exists() and stem not in index:
                index[stem] = (image_path, ann_path, split)
    return index


def read_manifest_stems() -> List[str]:
    return sorted(build_split_index().keys())


def convert_visdrone_gt_to_xyxy(_: Path, __: Path, out_dir: Path) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)
    for split in ['train', 'val', 'test']:
        gt_dir = DATA_ROOT / f'VisDrone2019-DET-{split}' / 'annotations'
        image_dir = DATA_ROOT / f'VisDrone2019-DET-{split}' / 'images'
        if gt_dir.exists() and image_dir.exists():
            REF.convert_visdrone_gt_to_xyxy(gt_dir, image_dir, out_dir)
...
def main() -> None:
    ...
    log('[Start] GroundingDINO baseline rerun on VisDroneSplit1000Guarded')
    convert_visdrone_gt_to_xyxy(DATA_ROOT / 'unused', DATA_ROOT / 'unused', OUTPUT_GT_XYXY_DIR)
