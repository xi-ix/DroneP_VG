...
EXP_ROOT = ROOT / 'experiment/exp36_small_target_alias_rerank_20260711'
LOG_DIR = EXP_ROOT / 'log'
RUN_LOG = LOG_DIR / 'run_log.txt'
SUMMARY_JSON = LOG_DIR / 'exp36_small_target_alias_rerank_20260711_summary.json'
PRED_ROOT = LOG_DIR / 'predictions'
BEST_CKPT = LOG_DIR / 'exp36_small_target_alias_rerank_20260711_best.pt'
...
def load_exp23_helpers():
    spec = importlib.util.spec_from_file_location('exp23_helpers_for_exp36', str(EXP23_SCRIPT))
    if spec is None or spec.loader is None:
        raise RuntimeError(f'Cannot import Exp23 helpers from {EXP23_SCRIPT}')
    module = importlib.util.module_from_spec(spec)
    sys.modules['exp23_helpers_for_exp36'] = module
...
def box_features(box: Box, width: int, height: int, gt_boxes: Sequence[Box] = ()) -> List[float]:
    ...
    overlap = [
        best_iou(box, gt_boxes, same_class_only=False) if gt_boxes else 0.0,
        confusion_iou(box, gt_boxes) if gt_boxes else 0.0,
    ]
...
def build_dataset(split: str, stems: Sequence[str], include_overlap_features: bool):
    features: List[List[float]] = []
    labels: List[float] = []
    weights: List[float] = []
    for stem in stems:
        width, height = image_size(split, stem)
        pred_boxes = read_pred(EXP35_PRED_ROOT / split / f'{stem}.txt')
        gt_boxes = read_gt(GT_ROOT / split / f'{stem}.txt')
        for box in pred_boxes:
            labels.append(label_and_weight(box, gt_boxes)[0])
            weights.append(label_and_weight(box, gt_boxes)[1])
            feature_gt = gt_boxes if include_overlap_features else ()
            features.append(box_features(box, width, height, feature_gt))
    return (
...
def train_model(x_train_raw: torch.Tensor, y_train: torch.Tensor, w_train: torch.Tensor, x_hold_raw: torch.Tensor, y_hold: torch.Tensor):
    ...
    model = AliasReranker(input_dim=x_train.shape[1]).to(DEVICE)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        model.eval()
        with torch.no_grad():
            hold_logits = model(x_hold.to(DEVICE))
            hold_loss = float(nn.functional.binary_cross_entropy_with_logits(hold_logits.cpu(), y_hold).item())
    ...
    model.to(DEVICE)
    model.eval()
    return model, ckpt['mu'], ckpt['sigma'], ckpt['best']


def calibrate_boxes(model, mu: torch.Tensor, sigma: torch.Tensor, split: str, stems: Sequence[str], alpha: float, out_dir: Path) -> None:
    if out_dir.exists():
    ...
    for stem in stems:
        ...
        features = [box_features(box, width, height, ()) for box in boxes]
        if features:
            with torch.no_grad():
        ...
        with (out_dir / f'{stem}.txt').open('w', encoding='utf-8') as f:
            for box, prob in zip(boxes, probs):
                score = alpha * box.score + (1.0 - alpha) * float(prob)
...
def ensure_required_predictions() -> None:
    missing = []
    for split in ['val', 'test']:
        pred_dir = EXP35_PRED_ROOT / split
        if not pred_dir.exists() or not any(pred_dir.glob('*.txt')):
            missing.append(str(pred_dir))
    if missing:
        raise RuntimeError('Missing Exp35 prediction directories: ' + ', '.join(missing))
...
def main() -> None:
    ...
    log(f'[Start] Exp36 small target hard-negative alias rerank device={DEVICE}')
    log(f'[Split] val_train={len(train_stems)} val_holdout={len(hold_stems)} test={len(test_stems)}')

    x_train, y_train, w_train = build_dataset('val', train_stems, include_overlap_features=False)
    x_hold, y_hold, _ = build_dataset('val', hold_stems, include_overlap_features=False)
    log(
        f'[Dataset] train_samples={len(y_train)} train_pos={int((y_train >= 0.5).sum())} '
        f'hold_samples={len(y_hold)} hold_pos={int((y_hold >= 0.5).sum())}'
    )
    ...
    best_metrics = None
    rows = []
    for alpha in [0.0, 0.25, 0.50, 0.65, 0.75, 0.90, 1.0]:
        out_dir = PRED_ROOT / 'val_holdout_search' / f'alpha_{alpha:.2f}'
        calibrate_boxes(model, mu, sigma, 'val', hold_stems, alpha, out_dir)
        metrics = EXP23.evaluate_detections(hold_gt_dir, out_dir, iou_thr=0.5)
        objective = 0.65 * metrics['map_05'] + 0.35 * metrics['acc_05']
        ...
        if best_metrics is None or objective > 0.65 * best_metrics['map_05'] + 0.35 * best_metrics['acc_05']:
            best_alpha = alpha
            best_metrics = metrics

    test_out = PRED_ROOT / 'test'
    calibrate_boxes(model, mu, sigma, 'test', test_stems, best_alpha, test_out)
    test_metrics = EXP23.evaluate_detections(GT_ROOT / 'test', test_out, iou_thr=0.5)
    exp35_test_metrics = EXP23.evaluate_detections(GT_ROOT / 'test', EXP35_PRED_ROOT / 'test', iou_thr=0.5)
    by_class = evaluate_by_class(GT_ROOT / 'test', test_out)
    summary = {
        'seed': SEED,
        'method': 'small-target hard-negative score calibration with multi-level alias hierarchy features',
        ...
        'exp35_test_metrics': exp35_test_metrics,
        'test_metrics': test_metrics,
        'test_by_class': by_class,
        'next_experiment_reminder': 'Next experiment should train an online fusion/calibration module combining GroundingDINO score, Exp30 scorer score, prompt/source features, geometry, alias hierarchy, and Exp36 calibrator score.',
    }
    ...
    log('[Reminder] Next experiment: online fusion/calibration module.')
