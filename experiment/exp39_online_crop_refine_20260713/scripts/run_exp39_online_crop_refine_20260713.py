...
EXP_ROOT = ROOT / 'experiment/exp39_online_crop_refine_20260713'
LOG_DIR = EXP_ROOT / 'log'
RUN_LOG = LOG_DIR / 'run_log.txt'
PRED_ROOT = LOG_DIR / 'predictions'
METADATA_ROOT = LOG_DIR / 'metadata'
GT_ROOT = ROOT / 'experiment/exp30_real_query_focal_rerank_20260711/log/gt_xyxy'
SUMMARY_JSON = LOG_DIR / 'exp39_online_crop_refine_20260713_summary.json'
...
CROP_REFINE_CLASSES = {1, 2, 3, 7, 8, 10}
CROP_REFINE_TOPK_PER_CLASS = 2
CROP_REFINE_SCALE = 2.4
CROP_REFINE_MIN_SIZE = 96
CROP_REFINE_BOX_THRESHOLD = 0.03
CROP_REFINE_TEXT_THRESHOLD = 0.15
CROP_REFINE_SCORE_BLEND = 0.55
...
def clip_candidate(box: Candidate, width: int, height: int) -> Candidate:
    return Candidate(
        box.class_id,
        ...
        box.source,
        box.source_name,
        gdino_score=box.gdino_score,
        query_match_score=box.query_match_score,
        small_area_bonus=box.small_area_bonus,
        query=box.query,
...
def crop_region_for_refine(box: Candidate, width: int, height: int) -> Tuple[int, int, int, int]:
    bw = max(1.0, box.x2 - box.x1)
    bh = max(1.0, box.y2 - box.y1)
    crop_w = max(bw * CROP_REFINE_SCALE, float(CROP_REFINE_MIN_SIZE))
    crop_h = max(bh * CROP_REFINE_SCALE, float(CROP_REFINE_MIN_SIZE))
    cx = (box.x1 + box.x2) / 2.0
    cy = (box.y1 + box.y2) / 2.0
    x1 = int(math.floor(cx - crop_w / 2.0))
    y1 = int(math.floor(cy - crop_h / 2.0))
    x2 = int(math.ceil(cx + crop_w / 2.0))
    y2 = int(math.ceil(cy + crop_h / 2.0))
    if x1 < 0:
        x2 -= x1
        x1 = 0
    if y1 < 0:
...
def prompts_for_refine_class(class_id: int) -> List[str]:
    prompts = [CLASS_NAMES[class_id]]
    prompts.extend(prompt for target_class, prompt in SMALL_ALIAS_PROMPTS if target_class == class_id)
    seen = set()
    unique = []
...
def prompt_for_refine_seed(seed: Candidate) -> str:
    if seed.source_name.startswith('gdino_alias:'):
        return seed.source_name.split(':', 1)[1]
    return CLASS_NAMES.get(seed.class_id, 'object')


def crop_refine_candidates(gdino_model, image_bgr: np.ndarray, candidates: Sequence[Candidate]) -> List[Candidate]:
    if not CROP_REFINE_ENABLED or not candidates:
        return []
    height, width = image_bgr.shape[:2]
    refine_seeds: List[Candidate] = []
    for class_id in sorted(CROP_REFINE_CLASSES):
        class_items = [
            box for box in candidates
            if box.class_id == class_id and box_area_ratio(box, width, height) <= AREA_THRESHOLDS.get(class_id, 0.02) * 2.5
        ]
        refine_seeds.extend(sorted(class_items, key=lambda item: item.score, reverse=True)[:CROP_REFINE_TOPK_PER_CLASS])

    refined: List[Candidate] = []
    for seed in refine_seeds:
        x1, y1, x2, y2 = crop_region_for_refine(seed, width, height)
        crop = image_bgr[y1:y2, x1:x2]
        if crop.size == 0:
            continue
        prompts = [prompt_for_refine_seed(seed)]
        boxes, scores, raw_class_ids = gdino_predict(
            gdino_model,
            crop,
            prompts,
            CROP_REFINE_BOX_THRESHOLD,
            CROP_REFINE_TEXT_THRESHOLD,
        )
        for box, score, raw_cid in zip(boxes, scores, raw_class_ids):
            if raw_cid is None:
                continue
            prompt_idx = int(raw_cid)
            ...
            mapped = Candidate(
                ...
                float(box[2]) + x1,
                float(box[3]) + y1,
                CROP_REFINE_SCORE_BLEND * float(score) + (1.0 - CROP_REFINE_SCORE_BLEND) * float(seed.score),
                seed.source,
                f'crop_refine:{prompt}',
                gdino_score=float(score),
                source_class_id=seed.class_id,
            )
            mapped = clip_candidate(mapped, width, height)
            if mapped.x2 - mapped.x1 >= 2.0 and mapped.y2 - mapped.y1 >= 2.0:
                refined.append(mapped)
    return deduplicate_candidates(refined, iou_threshold=0.90)
        ...
        for box, score, raw_cid in zip(boxes, scores, raw_class_ids):
            ...
            candidates.append(
                Candidate(
                    ...
                    gdino_score=float(score),
                    source_class_id=target_class,
                )
            )
    refined = crop_refine_candidates(gdino_model, image_bgr, candidates)
...
def score_candidates(
    ...
    for box, prob in zip(candidates, probs):
        query_match_score = float(prob)
        small_bonus = 0.0
        score = query_match_score
        if output_class in AREA_THRESHOLDS:
            with Image.open(image_path) as raw:
                width, height = raw.size
            small_bonus = 0.05 * small_area_prior(Candidate(output_class, box.x1, box.y1, box.x2, box.y2, box.score, box.source, box.source_name), width, height)
            score += small_bonus
        scored.append(
            Candidate(
                output_class,
                ...
                small_area_bonus=small_bonus,
                query=query,
                source_class_id=box.source_class_id or box.class_id,
            )
        )
    return scored
...
def write_metadata_jsonl(path: Path, image_path: Path, boxes: Sequence[Candidate]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with Image.open(image_path) as raw:
        width, height = raw.size
    with path.open('w', encoding='utf-8') as f:
        for rank, box in enumerate(sorted(boxes, key=lambda item: item.score, reverse=True), start=1):
            ...
            if box.source_name.startswith('crop_refine:'):
                source_type = 'crop_refine'
            elif box.source_name.startswith('gdino_alias:'):
                source_type = 'alias'
            else:
                source_type = 'base'
            source_prompt = box.source_name.split(':', 1)[1] if ':' in box.source_name else box.source_name
            payload = {
                'class_id': box.class_id,
                'class_name': CLASS_NAMES.get(box.class_id, 'unknown'),
                'source_class_id': box.source_class_id,
                'source_class_name': CLASS_NAMES.get(box.source_class_id, 'unknown'),
                ...
                'image_width': width,
                'image_height': height,
                'is_small_class': box.class_id in AREA_THRESHOLDS,
            }
...
def run_dataset(split: str, limit: Optional[int]) -> None:
    ...
    log(f'[Start] Exp39 online crop-refine inference split={split} limit={limit}')
    ...
    for idx, (part, stem) in enumerate(stems, start=1):
        ...
        write_metadata_jsonl(meta_dir / f'{stem}.jsonl', image_path, boxes)
    ...
    if eval_gt_dir.exists():
        ...
        summary = {
            ...
            'config': {
                ...
                'crop_refine': {
                    'enabled': CROP_REFINE_ENABLED,
                    'classes': sorted(CROP_REFINE_CLASSES),
                    'topk_per_class': CROP_REFINE_TOPK_PER_CLASS,
                    'scale': CROP_REFINE_SCALE,
                    'min_size': CROP_REFINE_MIN_SIZE,
                    'box_threshold': CROP_REFINE_BOX_THRESHOLD,
                    'text_threshold': CROP_REFINE_TEXT_THRESHOLD,
                    'score_blend': CROP_REFINE_SCORE_BLEND,
                },
                'metadata_fields': [
                    'class_id',
                    'source_class_id',
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Exp39 online crop-refine small-object grounding inference')
    ...
    RUN_LOG.write_text('', encoding='utf-8')
    log('[Start] Exp39 online single-image crop-refine inference')
    gdino_model = load_groundingdino_model()
    scorer_model, mu, sigma, vocab = load_scorer()
